...
public extension LKNameSpace where Base: UIFont {
    ...
    static func registerCustomFonts(types: [FontType] = FontType.allCases) {
        let fontUrls = types.compactMap({ type -> URL? in
            ...
            return Bundle.lk.default.url(forResource: source.name, withExtension: source.type)
        })
    ...
    static func familyURL(type: FontType) -> URL? {
        return Bundle.lk.default.url(forResource: type.source?.name, withExtension: type.source?.type)
    }
