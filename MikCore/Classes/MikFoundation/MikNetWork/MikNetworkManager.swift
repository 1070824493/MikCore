...
extension MikNetworkManager {
    ...
    private func request(url: String, method: MethodType, params: [String: Any]?, bodys: AnyObject?, responseType: ResponseType, success: @escaping (_ success: Any) -> (), failure: @escaping (_ error: MikError) -> ()) {
        ...
        func sendError(message: String = "url cannot be empty") {
            ...
            // "url cannot be empty"
            let model = MikResponseErrorInfoModel.init(message: message)
...
//        guard let url = url.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) else { return }
        ...
        if Self.CheckProxySetting(urlString: urlString) && !self.isDebug {
           sendError(message: "Please close the proxy and try again")
            return
        }
        ...
        if let toParams = params, toParams.count > 0 {
            guard let toRequestUrl = URL(string: urlString.encode(toParams)) else {
                sendError()
                return
            }
            requestUrl = toRequestUrl
        }
        ...
        if let toBodys = bodys, let jsonSting = String.toJson(toBodys) {
...
extension MikNetworkManager {
    ...
    func request(url: String,
        ...
        guard let url = url.mik.urlEncoding(), !url.isEmpty else {
            result?(.failure(.invalidURL))
            return nil
        }
