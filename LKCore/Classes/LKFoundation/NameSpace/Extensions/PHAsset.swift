...
public extension LKNameSpace where Base: PHAsset {
    ...
    static func isHEICImage(asset: PHAsset) -> Bool {
        guard asset.lk.type == .image else { return false }
...
//                let fileExtension = String(PHAssetResource.assetResources(for: theAsset).first?.originalFilename.split(separator: ".").last?.lowercased() ?? "mov")
            ...
            } else if theAsset.lk.type == .image {
                // 是否是HEIF、HEIC格式
                ...
                self.fetchImage(for: theAsset, size: {
                    ...
                    return theAsset.lk.previewSize
                }()) { (img, isDegraded) in
                    let imageData: Data? = {
                        let data = img?.lk.compressToBytes()
                        if isHeicImage {
...
public extension LKNameSpace where Base: PHAsset {
    ...
    static func exportVideo(for asset: PHAsset,
        ...
        _ = Self.fetchAVAsset(forVideo: asset) { avAsset, _ in
            if let set = avAsset {
                ...
                complete(nil, NSError(domain: "com.lk.error", code: -1, userInfo: [NSLocalizedDescriptionKey: "Export failed"]))
            }
        }
    }
    ...
    static func exportVideo(for asset: AVAsset,
        ...
        guard let exportSession = AVAssetExportSession(asset: asset, presetName: exportQuality.presetName) else {
            complete(nil, NSError(domain: "com.lk.error", code: -1000, userInfo: [NSLocalizedDescriptionKey: "Export failed"]))
            return
        }
