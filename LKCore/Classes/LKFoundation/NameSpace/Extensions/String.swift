...
// MARK: - Utils
public extension LKNameSpace where Base == String {
...
//            0x2700...0x27BF, // Dingbats
    ...
    /// 转换为数值类型
    var numberValue: NSNumber? { NumberFormatter.lk.numberValue(self.base) }
...
public extension LKNameSpace where Base == String {
    ...
    /// 从起始位置截取对应长度的子串
    /// - Parameter to: 子串结束位置
    func substring(to: Int) -> String { self.base.lk[0 ..< to] }
    
    /// 从指定位置截取到结束位置的子串
    /// - Parameter from: 子串起始位置
    func substring(from: Int) -> String { self.base.lk[from ..< self.base.count] }
...
public extension LKNameSpace where Base == String {
    ...
    /// - Note: 例如："1234567890".lk.formatter(splitLenths: [3, 3, 4], separator: "-") -> "123-456-7890"
