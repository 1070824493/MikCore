...
public enum MikTwoStepAlertViewType {
    case email(String)
    case phone(String)
}

fileprivate extension MikTwoStepAlertViewType {
    ...
    var textFieldStyle: MikTextFieldFormatterView.CustomFormatterStyle? {
        switch self {
...
// MARK: - Public meds
public extension MikTwoStepAlertView {

    @discardableResult
    static func alertTwoStepView(title: String?,
                                 twoStepType: MikTwoStepAlertViewType,
                                 showInView viewController: UIViewController,
                                 getCode: (() -> Void)?,
                                 submit: ((String?) -> Void)?) -> MikTwoStepAlertViewController
    {
        let alertView = MikTwoStepAlertView(type: twoStepType, title: title)

        let alertViewController = MikTwoStepAlertViewController(view: alertView)
...
public class MikTwoStepAlertViewController: MikAlertViewController {

    var twoStepAlertView: MikTwoStepAlertView? { self.customView as? MikTwoStepAlertView }
    ...
    public func updateCodeValidateResult(validateResult: MikTextFieldFormatterView.ValidateResult?) {
...
fileprivate class CodeProxy: CustomTextFieldVaildateProxy {
    ...
    static let kStyle: MikCore.MikTextFieldFormatterView.CustomFormatterStyle = .digits(maxLenth: 6, splitLenths: nil, separator: nil)

    required init() {
        super.init(style: CodeProxy.kStyle)
    }

    internal required init(style: MikTextFieldFormatterView.CustomFormatterStyle) { fatalError("init(style:)") }

}

public class MikTwoStepAlertView: UIView {
    ...
    private let type: MikTwoStepAlertViewType
    ...
    private lazy var cancelButton: MikButton = {
    ...
    private lazy var submitButton: MikButton = {
        let aBtn = MikButton(style: .fillRed)
        aBtn.titleLabel?.adjustsFontSizeToFitWidth = true
        aBtn.titleLabel?.font = UIFont.mik.font(.nunitoSansBold, size: 14)
    ...
    private lazy var getCodeButton: UIButton = {
        ...
        aBtn.setTitleColor(UIColor.mik.text(.hex0475BC, alpha: 0.5), for: .highlighted)
        aBtn.setTitleColor(UIColor.mik.text(.hex757575), for: .disabled)
    ...
    private lazy var valueTextView: MikTextFieldFormatterView = {
        let aTextView = MikTextFieldFormatterView(style: type.textFieldStyle, spacesTuple: (0, 0))
    ...
    private lazy var codeTextView: MikTextFieldFormatterView = {
        let aTextView = MikTextFieldFormatterView(style: CodeProxy.kStyle, spacesTuple: (36, 12))
    ...
    private lazy var activityIndicatorView: UIActivityIndicatorView = {
        ...
        aIndicatorView.color = UIColor.mik.general(.hex757575)
        return aIndicatorView
    }()

    required public init(type: MikTwoStepAlertViewType, title: String?) {
        self.type = type
        self.title = title
