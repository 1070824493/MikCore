//
//  LKTextFieldCustomDelegate.swift
//  LKCore
//
...
open class CustomTextFieldVaildateProxy: TextFieldDelegate {
    ...
    private let style: LKTextFieldFormatterView.CustomFormatterStyle
    
    required public init(style: LKTextFieldFormatterView.CustomFormatterStyle) {
        self.style = style
        super.init()
    }
    ...
    private func textFieldEditChanged(_ textField: UITextField) {
        let finalText: String? = {
            if let formatteTuple = self.style.formatteTuple {
                return textField.text?.lk.formatter(splitLenths: formatteTuple.splitLenths, separator: formatteTuple.separator)
            }
...
// MARK: - LKTextFieldFormatterView.CustomFormatterStyle
fileprivate extension LKTextFieldFormatterView.CustomFormatterStyle {
