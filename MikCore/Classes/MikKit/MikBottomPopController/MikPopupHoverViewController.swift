//
//  MikPopupHoverViewController.swift
//  MikCore
//
...
public typealias MikHoverViewController = MikBaseViewController & MikPopupHoverDataConfigures
public typealias MikHoverParentController = MikBaseViewController & MikPopupHoverParentConfig
public typealias MikPopupCompletion = () -> Void

public protocol MikPopupHoverParentConfig {
...
public extension MikPopupHoverParentConfig {
    
    var responderView: UIView? { nil }
    
}

public protocol MikPopupHoverDataConfigures {
...
public extension MikPopupHoverDataConfigures {
...
public enum MikPopupHoverStyle {
...
open class MikPopupHoverViewController: MikBaseViewController {
    ...
    private let hoverController: MikHoverViewController
    
    private weak var hoverParentViewController: MikHoverParentController?
        ...
        view = {
            ...
            aView.responderView = { [weak self] point in
                guard self?.contentView.frame.contains(point) ?? false else {
                    if let bPoint = self?.view?.convert(point, to: UIApplication.shared.mik.legacyKeyWindow) {
    ...
    required public init(hoverViewController: MikHoverViewController) {
...
// MARK: - Assistant
extension MikPopupHoverViewController {
    
    private func configure() {
        view.backgroundColor = UIColor.mik.general(.hex000000, alpha: 0)
        mik_navigationBarColor = .clear
    ...
    private func setupSubviews() {
        ...
        func setupLayers() {
            ...
            contentView.mik.setShadowWithCorners(size: CGSize(width: hoverController.contentWidth, height: 60),
                                                 roundingCorners: [.topLeft, .topRight],
                                                 cornerRadius: hoverController.contentCornerRadius ?? 0,
                                                 shadowColor: UIColor.mik.hex(0x000000, alpha: 0.2),
...
extension MikPopupHoverViewController {
    ...
    private func updateDimmingAlpha(contentViewHeight: CGFloat, animated: Bool) {
        ...
        let backgroundColor = UIColor.mik.general(.hex000000, alpha: self.hoverController.dimmingAlpha * percent)
    ...
    ///   - completion: 显示完成回调
    private func present(in viewController: UIViewController?, completion: MikPopupCompletion? = nil) {
    ...
    private func dismiss(completion: MikPopupCompletion? = nil) {
    ...
    ///   - completion: 显示完成回调
    private func moveToParentViewController(_ parentViewController: MikHoverParentController?, completion: MikPopupCompletion? = nil) {
    ...
    private func removeFromParentViewController(completion: MikPopupCompletion? = nil) {
        self.updateDimmingAlpha(contentViewHeight: 0, animated: true)
        UIView.animate(withDuration: 0.25, delay: 0, options: .curveEaseOut) {
            self.contentView.frame = {
                var oldFrame = self.contentView.frame
...
extension MikPopupHoverViewController {
    ...
    ///   - style: 显示类型
    ///   - completion: 显示完成回调
    public func show(in viewController: MikHoverParentController, style: MikPopupHoverStyle, completion: MikPopupCompletion? = nil) {
        self.style = style
    ...
    public func hidden(completion: MikPopupCompletion? = nil) {
        switch self.style {
        case .modal:
