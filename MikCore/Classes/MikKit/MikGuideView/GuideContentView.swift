...
class GuideContentView: UIView {
    ...
    required init(_ alphaRect: CGRect, _ title: NSAttributedString?, _ content: NSAttributedString?) {
        ...
        if topMargin > (height + arrowMargin) && downMargin > (UIScreen.mik.height - height - arrowMargin) {
        ...
        if title != nil {
            ...
            if self.arrowDirection == .up {
                self.titleLabel.frame = CGRect.init(x: snpMargin, y: snpMargin, width: UIScreen.mik.width - snpSuperMargin * 2 - snpMargin * 2, height: titleHeight)
            } else {
                self.titleLabel.frame = CGRect.init(x: snpMargin, y: arrowMargin + snpMargin, width: UIScreen.mik.width - snpSuperMargin * 2 - snpMargin * 2, height: titleHeight)
            }
        }
        if content != nil {
            self.contentLabel.attributedText = content
            self.addSubview(self.contentLabel)
            self.contentLabel.frame = CGRect.init(x: snpMargin, y: self.titleLabel.frame.maxY + contentMargin, width: UIScreen.mik.width - snpSuperMargin * 2 - snpMargin * 2, height: contentHeight)
        }
