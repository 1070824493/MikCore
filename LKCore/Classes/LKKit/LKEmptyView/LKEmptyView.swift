...
fileprivate extension LKEmptyValuesConfig.ButtonValues.Style {
    ...
    var titleColor: UIColor {
        switch self {
        case .redFill, .blackFill: return UIColor.lk.text(.hexFFFFFF)
        case .redBorder: return UIColor.lk.text(.hexCF1F2E)
        case .blackBorder: return UIColor.lk.text(.hex1B1B1B)
        case .refresh: return UIColor.lk.text(.hex0475BC)
        }
    }
    
    var highlightTitleColor: UIColor {
        switch self {
        case .redFill: return UIColor.lk.text(.hexFFFFFF)
        case .blackFill: return UIColor.lk.text(.hexFFFFFF)
        case .redBorder: return UIColor.lk.text(.hexEB003B)
        case .blackBorder: return UIColor.lk.text(.hex1B1B1B)
        case .refresh: return UIColor.lk.text(.hex0475BC, alpha: 0.5)
        }
    }
    
}

public struct LKEmptyValuesConfig {
    ...
    public struct TitleValues {
        ...
        public init(text: String?, font: UIFont = UIFont.lk.font(.nunitoSansBold, size: 16), textColor: UIColor = UIColor.lk.text(.hex1B1B1B), textAlignment: NSTextAlignment = .center) {
            self.text = text
            self.font = font
    ...
    public struct MessageValues {
        ...
        public init(text: String?, font: UIFont = UIFont.lk.font(size: 12), textColor: UIColor = UIColor.lk.text(.hex757575), textAlignment: NSTextAlignment = .center) {
            self.text = text
            self.font = font
            self.textColor = textColor
            self.textAlignment = textAlignment
...
public class LKEmptyView: UIView {
    
    public typealias CustomActionButtonCallback = (UIButton) -> Void
    public typealias ActionHandler = () -> Void
    
    private let valuesConfig: LKEmptyValuesConfig
    ...
    private lazy var actionButtonView: MEButtonView = {
        ...
        if let style = valuesConfig.buttonValues?.style {
            ...
            aBtnView.actionBtn.setBackgroundImage(UIImage.lk.image(style.backgroundColor), for: .normal)
            aBtnView.actionBtn.setBackgroundImage(UIImage.lk.image(style.highlightBackgroundColor), for: .highlighted)
    ...
    public required init(valuesConfig: LKEmptyValuesConfig, offsetCenterY: CGFloat? = nil, handler: ActionHandler? = nil) {
