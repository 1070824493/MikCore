//
//  LKTextFieldFormatterView.swift
//  LKCore
//
...
public extension LKTextFieldFormatterView {
    ...
    enum CustomFormatterStyle {
        ...
        var formatteTuple: LKFormatterTextField.FormatterTuple? {
            switch self {
    ...
    var textField: LKFormatterTextField { self.textFieldView.textField }
    
}


// MARK: - extensions
fileprivate extension LKTextFieldFormatterView.ValidateResult {
    ...
    var attributeMessage: NSAttributedString {
        ...
        if let message = message {
            let msgAttribute = NSAttributedString(string: "\(message)", attributes: [.font : UIFont.lk.font(size: 12), .foregroundColor : tinColor])
...
public class LKTextFieldFormatterView: UIView {
    ...
    private let formatteTuple: LKFormatterTextField.FormatterTuple?
    ...
    required public init(formatteTuple: LKFormatterTextField.FormatterTuple?, spacesTuple: SpacesTuple = (28, 4)) {
...
extension LKTextFieldFormatterView {
    ...
    private func setupStackArrangedSubviews() {
        if let title = title, !title.isEmpty {
            ...
            if mStackView.arrangedSubviews.contains(titleView) {
                mStackView.lk.removeArrangedSubviewCompletely(titleView)
            }
        }
    }       
...
fileprivate class TitleView: UIView {
    ...
    private(set) lazy var titleLabel: UILabel = {
        let aLabel = UILabel()
        aLabel.font = UIFont.lk.font(.nunitoSans, size: 12)
...
fileprivate class TextFieldView: UIView {
    ...
    private(set) lazy var textField: LKFormatterTextField = {
        let aTextField = LKFormatterTextField(formatteTuple: formatteTuple)
        aTextField.font = UIFont.lk.font(size: 16)
        aTextField.textColor = UIColor.lk.text(.hex1B1B1B)
    ...
    required init(formatteTuple: LKFormatterTextField.FormatterTuple?) {
        self.formatteTuple = formatteTuple
        super.init(frame: .zero)
...
fileprivate class ValidateView: UIView {
    ...
    var validateResults: [LKTextFieldFormatterView.ValidateResult]? {
        didSet {
    ...
    private lazy var titleLabel: UILabel = {
        let aLabel = UILabel()
        aLabel.font = UIFont.lk.font(.nunitoSans, size: 12)
        aLabel.textColor = UIColor.lk.text(.hexEB003B)
