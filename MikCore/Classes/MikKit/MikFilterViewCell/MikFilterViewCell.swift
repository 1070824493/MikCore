...
public class MikFilterViewCell: UIView {
    ...
    public var isShowLayerLine = false {
        didSet {
            if isShowLayerLine {
                ...
                self.bgButton.layer.borderColor = UIColor.mik.general(.hexAEAEAE).cgColor
    ...
    public lazy var bgButton: UIButton = {
        ...
        button.setImage(UIImage.mik.image(.white), for: .normal)
        button.setImage(UIImage.mik.image(UIColor.mik.general(.hexF6F6F6)), for: .selected)
        return button
    }()
    
    public lazy var selectButton: UIButton = {
        let button = UIButton.init(type: .custom)
        button.setImage(UIImage.image("mik_number_plus"), for: .normal)
        button.setImage(UIImage.image("mik_x"), for: .selected)
    ...
    private lazy var titleLabel: UILabel = {
        ...
        item.textColor = UIColor.mik.text(.hex1B1B1B)
        return item
    }()
...
extension MikFilterViewCell {
    
    public func setTitle(_ title: String, number: Int) {
        let attr = NSMutableAttributedString(string: title, attributes: [NSAttributedString.Key.font : UIFont.mik.font(size: 16.rate), NSAttributedString.Key.foregroundColor : UIColor.mik.general(.hex1B1B1B)])
        if number > 0 {
            let countAttr = NSAttributedString(string: "（\(number)）", attributes: [NSAttributedString.Key.font : UIFont.mik.font(.nunitoSans, size: 16.rate), NSAttributedString.Key.foregroundColor : UIColor.red])
