//
//  MikPickerTestViewController.swift
//  MikCore
//
...
class MikPickerTestViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {        
        ...
        case 0:
            let items = Array(0...30).map({ MikPickerItem(title: "选项\($0)") })
            
            MikPickerViewController.pickerView(showIn: self, title: "这是标题这是标题这是标题", items: [items]) { (selectedIndexPaths) in
                var text = "选择了"
                selectedIndexPaths.forEach({ text += "第\($0.section)列, 第\($0.row)行" })
                print(text)
            }
        case 1:
            let items = Array(0...30).map({ MikPickerItem(title: "选项\($0)", font: randomFont, color: randomColor, isSelected: $0 == 10) })
            
            MikPickerViewController.pickerView(showIn: self, title: "这是标题这是标题这是标题这是标题这是标题这是标题这是标题这是标题这是标题", items: [items, items, items], customConfirm: { (btn) in
