...
public enum LKOptionsType {
    ...
    var options: KingfisherOptionsInfo? {
        ...
        case .thumbnail(let size):
            return [.processor(LKDownsamplingImageProcessor(size: size)),
...
fileprivate extension Resource {
    ...
    /// - Returns: 缩略图地址
    func asThumbURL(type: LKOptionsType) -> Resource? {
        let size: CGSize? = {
            switch type {
...
private struct LKDownsamplingImageProcessor: ImageProcessor {
    ...
    public init(size: CGSize) {
        ...
        self.identifier = "com.onevcat.Kingfisher.LKDownsamplingImageProcessor(\(size))"
    }
...
public extension LKNameSpace where Base: UIImageView {
    ...
    /// - Returns: 下载任务
    @discardableResult
    func setImage(type: LKOptionsType = .none,
                  url: URL?,
    ...
    func setImage(type: LKOptionsType = .none,
                  ...
                  progressBlock: DownloadProgressBlock? = nil,
                  completionHandler: ((Result<RetrieveImageResult, KingfisherError>) -> Void)? = nil) -> DownloadTask? {
        guard let url = url?.lk.urlEncoding(["#","%"]), let bUrl = URL(string: url) else {
            self.base.image = placeholder
            return nil
        }
    ...
    func setImage(with url: URL?,
        ...
        LKKingfisher.image(videoUrl: url, size: size, seconds: seconds) {
            self.base.image = placeholder
...
public extension LKNameSpace where Base: UIButton {
    ...
    /// - Returns: 下载任务
    @discardableResult
    func setBackgroundImage(type: LKOptionsType = .none,
                            url: URL?,
                            for state: UIControl.State,
                            placeholder: UIImage? = nil,
                            progressBlock: DownloadProgressBlock? = nil,
