//
//  LKTextFieldFormatterViewViewController.swift
//  LKCore
//
...
private let phoneStyle: LKTextFieldFormatterView.CustomFormatterStyle = .phone(lenth: 11)

class LKTextFieldFormatterViewViewController: LKBaseViewController {
    ...
    // LKTextField
    private lazy var normalTextField: LKFormatterTextField = {
        let aTextField = LKFormatterTextField(formatteTuple: ([4], "-"))
        aTextField.backgroundColor = .white
        aTextField.attributedPlaceholder = NSAttributedString(string: "xxxx-xxxx...", attributes: [.foregroundColor : UIColor.lk.text(.hexAEAEAE)])
        return aTextField
    }()
    
    // 手机号格式化 + 输入校验（可自定义校验规则）
    private lazy var phoneTextFieldFormatterView: LKTextFieldFormatterView = {
        let aTextFieldFormatterView = LKTextFieldFormatterView(style: phoneStyle)
        ...
        aTextFieldFormatterView.textField.attributedPlaceholder = NSAttributedString(string: "点击屏幕切换更多样式", attributes: [.foregroundColor : UIColor.lk.text(.hexAEAEAE)])
    ...
    // 自定义格式化样式 + 密码框
    private lazy var customFieldFormatterView: LKTextFieldFormatterView = {
        let aTextFieldFormatterView = LKTextFieldFormatterView(formatteTuple: ([1, 2, 3, 4], "/"), spacesTuple: (0, 0))
        aTextFieldFormatterView.title = "Custom formatter"
        aTextFieldFormatterView.textField.isSecureTextEntry = true
        aTextFieldFormatterView.textField.setupSecureTextEntry(image: UIImage.image("lk_text_success"), selectedImage: UIImage.image("lk_text_error"), handler: nil)
        return aTextFieldFormatterView
    }()
