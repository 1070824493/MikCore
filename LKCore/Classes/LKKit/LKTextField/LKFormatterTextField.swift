...
public extension Reactive where Base: LKFormatterTextField {
...
open class LKFormatterTextField: UITextField {
    ...
    open override var text: String? {
        didSet {
            ...
            super.text = text?.lk.formatter(splitLenths: formatteTuple.splitLenths, separator: formatteTuple.separator)
        }
    }
    ...
    required public init(frame: CGRect = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width - 48, height: 48),
...
// MARK: - Assistant
fileprivate extension LKFormatterTextField {
    
    private func setupNotifications() {
        guard isFormatter, let formatteTuple = self.formatteTuple else { return }
        
        NotificationCenter.default.addObserver(forName: UITextField.textDidChangeNotification, object: self, queue: OperationQueue.main) { [weak self] (noti) in
            guard let textField = noti.object as? UITextField, textField === self else { return }
            ...
            textField.text = textField.text?.lk.formatter(splitLenths: formatteTuple.splitLenths, separator: formatteTuple.separator)
        }
    }
