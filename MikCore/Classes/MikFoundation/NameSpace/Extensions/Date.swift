...
public extension MikNameSpace where Base == Date {
    
    /// eg. 2021.4.25 -> Date().mik.dateOffset(year: 1, month: -1) -> 2022.3.25
    ...
    static func timeInterval(_ dateString: String?, dateTransFormType: DateTransformType = .ISO8601Date) -> TimeInterval? {
        ...
        let format = DateFormatter.mik.formatter(dateTransFormType.dateFormatter)
    ...
    static func dateString(_ timestamp: TimeInterval?, formatter: String) -> String? {
        ...
        return DateFormatter.mik.formatter(formatter).string(from: date)
    }
...
public extension MikNameSpace where Base == Date {
    ...
    func startOfMonth(in calendar: Calendar = .current) -> Date {
        return calendar.date(from: calendar.dateComponents([.year, .month], from: calendar.startOfDay(for: self.base)))?.mik.startOfDay(in: calendar) ?? self.base
    }
    
    /// 一个月的最后一天
    func endOfMonth(in calendar: Calendar = .current) -> Date {
        return calendar.date(byAdding: DateComponents(month: 1, day: -1), to: self.base.mik.startOfMonth(in: calendar))?.mik.endOfDay(in: calendar) ?? self.base
    }
