...
public extension LKNameSpace where Base: UIColor {
    ...
    private static func name(_ name: String, alpha: CGFloat? = nil) -> UIColor {
        return UIColor { (trait) -> UIColor in
            if let alpha = alpha {
                return UIColor(named: name, in: Bundle.lk.default, compatibleWith: trait)!.withAlphaComponent(alpha)
            } else {
                return UIColor(named: name, in: Bundle.lk.default, compatibleWith: trait)!
            }
        }
    }
