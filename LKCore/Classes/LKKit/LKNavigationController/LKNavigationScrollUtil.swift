...
class LKNavigationScrollUtil {
    ...
    init(viewController: UIViewController) {
        vc = viewController
        scrollView = LKNavigationScrollUtil.findScrollView(view: viewController.view)
        obseverScrollView()
    }
    ...
    func updateTabbarHiddenStatus(isHidden: Bool) {
        ...
        UIView.animate(withDuration: kTabbarHiddenAnimationDuration) {
            if isHidden {
                ...
                frame.origin.y = UIScreen.lk.height
                tabbar.frame = frame

            } else {
                var frame = tabbar.frame
                frame.origin.y = UIScreen.lk.height - UIViewController.lk.safeAreaMax.bottom
                tabbar.frame = frame
            }
        } completion: { _ in
            self.isTabbarHidden = tabbar.frame.minY == UIScreen.lk.height
        ...
        scrollView.rx.contentOffset
            .subscribe(onNext: { [weak self] _ in
                ...
                if deltaY > 0 {
                    ...
                    fy = UIScreen.lk.height - UIViewController.lk.safeAreaMax.bottom + min(deltaY - 44, UIViewController.lk.safeAreaMax.bottom)
                } else {
                    if self.isTabbarHidden == false {
                        return
                    }
                    // 手指往下滑动
                    fy = UIScreen.lk.height - min(abs(deltaY) - 44, UIViewController.lk.safeAreaMax.bottom)
                }
