...
/// 文件资源类型
public enum LKDocumentSourceType: String, CaseIterable {
    case image = "public.image", video = "public.movie", pdf = "com.adobe.pdf"
}

/// 文件类型
public enum LKMediaPreviewType : Int {
    /// 图片
    case image
    /// 视频
    case video
...
public enum LKMapsType {
    ...
    var url: URL? {
        ...
        case .apple(let address, let latitude, let longitude):
            ...
            guard let urlText = "http://maps.apple.com/?q=\(q)&sll=\(sll))&z=10&t=s".lk.urlEncoding() else {
                return nil
            }
        ...
        case .google(let address, let latitude, let longitude):
            ...
            guard let urlText = "https://www.google.com/maps/?q=\(q)&center=\(center)".lk.urlEncoding() else {
                return nil
            }
            return URL(string: urlText)
        ...
        case .waze(let address, let latitude, let longitude):
            ...
            guard let urlText = "https://waze.com/ul?q=\(q)&ll=\(ll)&navigate=yes".lk.urlEncoding() else {
                return nil
            }
            return URL(string: urlText)
        }
...
/// 展示资源数据
public typealias LKMediaPreviewData = (mediaValue : Any?,mediaType : LKMediaPreviewType)
...
fileprivate extension ZLPhotoConfiguration {
    ...
    /// 更新配置
    static func updateConfiguration(type: LKMediaType) {
...
public class LKMediaUtil {
    ...
    ///   - complete: 完成回调
    public static func openAlbum(at viewController: UIViewController?, style: LKMediaType.AlbumStyle, isAllowTakePhoto: Bool = true, selectedAssets: [PHAsset]? = nil, complete: (([UIImage], [PHAsset], Bool) -> Void)?) {
        guard let viewController = viewController else { return }
    ...
    ///   - types: 资源类型
    ///   - proxy: 代理
    public static func openFile(at viewController: UIViewController?, types: [LKDocumentSourceType] = LKDocumentSourceType.allCases, proxy: LKDocumentProxy) {
        guard let viewController = viewController else { return }
    ...
    ///   - mapTypes: 地图类型
    ///   - failure: 失败回调
    public static func openMaps(at viewController: UIViewController?, title: String? = nil, messages: String? = nil, mapTypes: [LKMapsType]?, failure: (() -> Void)? = nil) {
...
// MARK: - Preview
public extension LKMediaUtil {
    
    /// 浏览图片或视频 数据传LKMediaPreviewData数组
    /// - Parameters:
    ///   - viewController: 当前控制器
    ///   - datas: LKMediaPreviewData数组
    ///   - previewData: 当前显示的多媒体
    ///   - showSelect: 是否显示选择按钮
    static func preview(at viewController: UIViewController?, datas: [LKMediaPreviewData], previewData: LKMediaPreviewData?, showSelect: Bool = false) {
        guard let viewController = viewController , !datas.isEmpty else { return }
        /// 展示数据源
        var list = [Any]()
        /// 每个数据对应的文件类型
        var previewTypeList = [LKMediaPreviewType]()
        var index : Int = 0
    ...
    ///   - index: 需要展示的位置
    ///   - showSelect: 是否显示选择按钮
    private static func preview(at viewController: UIViewController, datas: [Any], previewTypeList: [LKMediaPreviewType], index: Int = 0, showSelect: Bool = false) {
        ...
        let previewVC = ZLImagePreviewController(datas: datas, index: index < 0 ? 0 : index, showSelectBtn: showSelect, showBottomView: showSelect, urlType: { (url) -> ZLURLType in
            ...
            imgView.lk.setImage(type: .none, url: url, placeholder: nil) { (receivedSize, totalSize) in
...
// MARK: - LKDocumentProxy
public class LKDocumentProxy: NSObject, UIDocumentPickerDelegate {
