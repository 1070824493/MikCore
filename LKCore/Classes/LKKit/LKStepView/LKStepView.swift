...
public class LKStepView: UIView {

    public var takeStepBlock: ((_ title: String, _ tag: Int) -> ())?
    
    private let scrollView: UIScrollView = UIScrollView()
    
    private var buttons = [LKStepButton]()
    ...
    public required init(titles: [String]) {
        ...
        for (index, title) in titles.enumerated() {
            ...
            var width = title.lk.boundingRect(font: stepButton.titleLabel?.font ?? UIFont.lk.font(.nunitoSansBold, size: 14.rate), limitSize: CGSize.init(width: CGFloat(MAXFLOAT), height: 43.rate)).width + 40.rate
            
            if titles.count == 2 {
                width = UIScreen.lk.width / 2.0 - 24.rate
            }
    ...
    private func createButton(title: String) -> LKStepButton {
        let button = LKStepButton.init(type: .custom)
        button.contentHorizontalAlignment = .left
        button.titleLabel?.font = UIFont.lk.font(.nunitoSansBold, size: 14.rate)
        button.setTitleColor(UIColor.lk.text(.hexF8D2CB), for: .normal)
        button.setTitleColor(UIColor.lk.text(.hexCF1F2E), for: .selected)
        button.setTitleColor(UIColor.lk.text(.hexCDCDCD), for: .disabled)
...
extension LKStepView {
    
    @objc private func buttonAction(btn: LKStepButton) {
        self.takeStepBlock?(btn.currentTitle ?? "", btn.tag)
    }
    
    public func setStepIndex(index: Int, enabledIndex: Int) {
        
        if let currentBtn = self.buttons.lk[safe: index] {
            let minX: CGFloat = 0
    ...
    public func setStepIndex(index: Int, enabledIndexs: [Int]) {
        ...
        if  let currentBtn = self.buttons.lk[safe: index] {
            ...
            enabledIndexs.forEach { i in
                if let button = self.buttons.lk[safe: i] {
                    button.isEnabled = true
                }
            }
