...
open class MikTextViewCell: UIView {
    ...
    /// 是否正在输入
    public lazy var outputEditing: Driver<Void> = {
        return Observable.merge(self.textField.rx.controlEvent(.editingDidBegin).asObservable(),
                                self.textView.rx.didBeginEditing.asObservable())
    ...
    lazy private var bgTextView: UIView = {
        ...
        bgView.layer.borderColor = UIColor.mik.general(.hexAEAEAE).cgColor
        return bgView
    }()
    
    lazy private var placeholderLabel : UILabel = {
        let label = UILabel()
        label.font = UIFont.mik.font(.nunitoSans, size: 16.rate)
...
// MARK: - init
    public override init(frame: CGRect) {
        ...
        placeholderLabel.snp.makeConstraints { (maker) in
            maker.left.equalToSuperview().offset(textView.textContainerInset.left)
            maker.width.equalToSuperview().offset(-(textView.textContainerInset.left + textView.textContainerInset.right))
            maker.top.equalToSuperview().offset(textView.textContainerInset.top)
        }
    }
    
    private func setupTextFiledPlaceholder() {
        self.textField.attributedPlaceholder = NSAttributedString(string: self.placeholder ?? "", attributes: [NSAttributedString.Key.foregroundColor : self.placeholderColor ?? UIColor.mik.text(.hexEAEAEA), NSAttributedString.Key.font : self.placeholderFont ?? UIFont.mik.font(size: 16.rate)])
    }
